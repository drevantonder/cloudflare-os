...
- **Connections:** when a user connects Google (or signs in and later connects it), the scopes for
  the selected resources (Gmail, Drive, Docs, Sheets, Calendar, or BigQuery — see below) are requested so
...
You'll need to enable the Google APIs that you want to use. Currently supported: Gmail, Google Drive, Google Docs, Google Sheets, Google Calendar, and BigQuery.
...
The Google Drive API powers the account-wide read-only Drive connection and the document and
spreadsheet resource pickers. Document reads and edits still go through the Google Docs API, and
spreadsheet reads go through the Google Sheets API.
...
- `drive.readonly` to list, search, download, and export every file visible to an account-wide
  Google Drive connection. This does not permit uploads or other changes.
...
5. Choose a Google resource type: Gmail, Google Drive, Google Doc, Google Spreadsheet, Google Calendar, or BigQuery.
